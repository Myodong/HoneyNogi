...
function Select-DgDifficultyWord {
  ...
  foreach ($word in $Words) {
    ...
    # 선택) / '이컪울'(s5, 같은 화면) / '이己1움'(s5, 2026-09-16 19:43 페카 심층 옵션 1273x718 -
    # 배율 4·3·2 는 빈 판독이고 s5 만 이 이형이라 3회 미탐 → 코드 1 × 3(커스텀 자동 재시작 포함) 정지.
    # 오류 캡처 3장 동일 문자열·좌표 (662,129), 보관 심층 옵션 22장은 전부 정상 '어려움'/'매우' 판독).
    # 전부 오류 캡처 오프라인 재현 실측. 이형에도 아래 짝 제외·앵커/HardX(OR 조건) 검증을 동일하게
    # 적용한다 - 리뷰 승인)
    $textMatches = ([string]$word.Text -eq $Key) -or
                   ($Key -eq '어려움' -and (@('이려움', '이컪움', '이컪울', '이己1움') -contains [string]$word.Text))
...
function Set-DgOptionStage {
  ...
  for ($try = 1; $try -le 8; $try++) {
    ...
    # 사용자 조작 중이면 클릭하지 않고 끝날 때까지 기다립니다 (2026-09-08 09:07 실사고 -
    # v2.1.6 배포 직후: 조작 중 Click-ScreenPoint 가 클릭을 버리는데 이 루프는 그걸 모른 채
    # "카드 클릭" 로그를 찍고 3회를 7초 만에 소진 → 코드 4 정지. 어제 Set-DgToggleCard 에
    # 넣은 것과 같은 기전의 두 번째 실사고). 계약은 Set-DgToggleCard 와 동일: 시도 회수($try)
    # 미소모 + 대기 + **재판독부터** (대기 후 옛 좌표 강행 클릭 금지 - v2.1.1 stale-click
    # 계약). Focus-Game 의 ALT 주입 전에 양보해야 조작 중 포커스를 뺏지 않습니다. 보조 판정
    # 잠금($observedTried)도 풀어 새 화면 상태에서 다시 1회 허용합니다 (Codex 조건 - 아래
    # 클릭 후 초기화와 같은 '화면 상태별 1회' 계약. 잠긴 채 재개하면 제목 불명 지속 시
    # 보조 판정 없이 남은 회수를 소진).
    ...
    $clickHow = $(if ($target.ContainsKey('Screen')) { '글자 탐색' } else { '예비 좌표' })
    ...
    # 클릭이 **실제로 나갔을 때만** '클릭'으로 기록합니다 - 생략된 클릭을 눌렀다고 적으면
    # 로그가 거짓이 되어 오늘 사고처럼 "3번 눌렀는데 왜 안 바뀌지?"로 진단이 헛돕니다.
    # N/3 은 실제 클릭 수가 아니라 **시도 예산** - 커서 확인 실패도 소모합니다 (유한 종료
    # 유지, Codex 표). lastClickPerformed 는 입력 전송 여부일 뿐, 게임 반영은 아래 제목
    # 재판독이 확인합니다.
    if ($script:lastClickPerformed) {
      Write-RunLog "$LogTag 구역 ${Stage} 카드 클릭 - $clickHow (시도 ${clicks}/3)"
    } elseif ($script:lastClickSkipReason -eq 'user-active') {
      # 사전 게이트와 클릭 사이의 경합 창(Focus/좌표 탐색 ~수백 ms)에 조작이 시작된 경우 -
      ...
      # '커서 미확인' - '실패' 단어를 쓰면 GUI 심각도 판정이 오류 줄로 칠합니다 (test_log_severity 대조).
      # 뒷문구 없음: 3/3 이면 다음 회전은 재시도가 아니라 한도 종료라 "다음 회전에서 재시도"가 거짓 (Codex)
      Write-RunLog "$LogTag 구역 ${Stage} 카드 클릭 건너뜀 (커서 미확인, 시도 ${clicks}/3)"
    }
...
function Get-ScaledScreenPoint {
  ...
  #   픽셀 판정은 한 번에 표본을 수십 회(현재 63회 - 2026-08-20 dx 촘촘화) 찍으면서
  #   `catch { continue }` 로 예외를 삼킵니다 (예: Test-DifficultySelectedAt). 여기에 몇 초짜리
  #   대기를 두면 그 대기가 표본 수 × 재시도 횟수만큼 곱해져 **수십 분을 로그 한 줄 없이
  #   태웁니다** (2026-08-09 4차 점검 실측: 당시 27표본 × 25초 × 5회전 ≈ 56분 - 63표본이면
  #   더 심함). 복원 대기는 클릭 1회에 1번만 도는 Click-GamePoint 로
...
function Wait-UserYieldEnd {
  ...
  while (Test-UserRecentlyActive) {
    ...
    if (((Get-Date) - $yieldHeartbeat).TotalSeconds -ge 60) {
      $yieldHeartbeat = Get-Date
      Write-RunLog "[안내] 사용자 조작이 계속되고 있어 ${Context} 대기 중입니다 ($([int]$yieldClock.Elapsed.TotalSeconds)초 경과)"
    }
  }
  $yieldClock.Stop()
...
function Get-KoreanObjectParticle {
  # 로그 문구용 목적격 조사 - 한글 마지막 글자의 받침 유무로 '을'/'를' (2026-09-08 실기 로그
  # "구역 2-2 전환를" 어색함 수정). 한글이 아니면 '을(를)'.
  param([string]$Word)
  if ([string]::IsNullOrEmpty($Word)) { return '을(를)' }
  $code = [int][char]$Word[$Word.Length - 1]
  if ($code -lt 0xAC00 -or $code -gt 0xD7A3) { return '을(를)' }
  if ((($code - 0xAC00) % 28) -ne 0) { return '을' }
  return '를'
}
...
function Move-CursorOutsideGame {
  ...
  if ($null -ne $userYieldClock) {
    # 이 양보도 시간 상한 루프의 마감 연장 대상입니다 (Codex: Wait-UserYieldEnd 를 안 거치는
    # 유일한 양보라 누락되면 판독 직전 대피에서 보낸 시간이 마감에서 빠짐)
    $script:userYieldTotalMs = [double]$script:userYieldTotalMs + $userYieldClock.Elapsed.TotalMilliseconds
...
function Invoke-ClickUntil {
  ...
  $deadline = (Get-Date).AddSeconds($TimeoutSeconds)
  # 양보 기준값 - 사용자 조작으로 기다린 시간만큼 마감을 늘립니다 (2026-09-08 전수 분류:
  ...
  while ((Get-Date) -lt (Get-YieldAdjustedDeadline -Deadline ([ref]$deadline) -SeenYieldMs ([ref]$seenYieldMs))) {
    ...
    # 사용자 조작 중이면 클릭하지 않고 끝날 때까지 기다린 뒤 마감을 연장하고 **조건 확인부터**
    # 다시 합니다 (대기 중 목표 화면에 도달했으면 위 Condition 이 잡음 - 옛 판단으로 클릭 금지)
    if (Test-UserRecentlyActive) {
      Wait-UserYieldEnd -Game $Game -Context $Description
      $deadline = $deadline.AddMilliseconds([double]$script:userYieldTotalMs - $seenYieldMs)
      $seenYieldMs = [double]$script:userYieldTotalMs
      continue
    }
    # 이번 회전에 클릭이 없을 수도 있어(소스 미확인·예비 조건 불충족) 생략 원인 메타를 비웁니다 -
    # 지난 호출의 'user-active' 가 남아 아래 백업 분기를 오발동하면 재클릭 간격 없이 도는 바쁜 루프
    $script:lastClickSkipReason = ''
    ...
    # 경합 창 백업: 게이트 통과 후 클릭 직전에 조작이 시작돼 생략됐으면 같은 양보 경로
...
function Invoke-VerifiedContentExit {
  ...
  while ((Get-Date) -lt (Get-YieldAdjustedDeadline -Deadline ([ref]$verifyDeadline) -SeenYieldMs ([ref]$seenYieldMs))) {
    ...
    $fieldStreak = 0
    # 사용자 조작 중이면 재클릭하지 않고 기다린 뒤 마감을 연장하고 팝업·필드 증거 판독부터 다시
    # (2026-09-08 전수 분류 - 40초 상한이 조작 중 그대로 소모돼 '복귀 미확인' 정지)
    if (Test-UserRecentlyActive) {
      Wait-UserYieldEnd -Game $Game -Context '나가기 확인'
      $verifyDeadline = $verifyDeadline.AddMilliseconds([double]$script:userYieldTotalMs - $seenYieldMs)
      $seenYieldMs = [double]$script:userYieldTotalMs
      continue
    }
    $script:lastClickSkipReason = ''   # 상태 기반 재클릭은 클릭이 없을 수도 있어 지난 메타 잔존 방지
    & $ReclickIfSource
    if (-not $script:lastClickPerformed -and $script:lastClickSkipReason -eq 'user-active') {
      Wait-UserYieldEnd -Game $Game -Context '나가기 확인'
      $verifyDeadline = $verifyDeadline.AddMilliseconds([double]$script:userYieldTotalMs - $seenYieldMs)
...
function Test-DifficultySelectedAt {
  ...
  # dx 그물 촘촘화 3열 → 7열 (2026-08-20 09:07 실사고 - 1908 네이티브 심층 선택 화면의
  # 선택 강조는 '노란 채움'이 아니라 **노란 테두리 + 어두운 채움**이라, 3열 그물이 얇은
  # 테두리를 2/27 표본밖에 못 잡아 이미 선택된 알약을 미선택으로 오판 → 3회차 연속 정지.
  # 오류 캡처 재샘플링이 로그와 완전 일치(2/27)했고, 7열로 9/63 통과. X 외연 ±12·색 조건·
  # 문턱(≥3) 불변이라 기존 통과 화면은 단조성으로 계속 통과. 교차검증(Codex): 보관 캡처
  # 85장 - 비선택 0/63, 선택 12~25/63, 1~2히트 경계 사례 없음. 오탐 없음은 '안정된 화면 +
  # 커서 대피 성공' 조건부 명제 - 대피는 위에서 이미 수행됨.
  foreach ($dy in @(-18, -14, -10, -6, 0, 6, 10, 14, 18)) {
    $rowMaxBright = 0
    $rowMaxSat = 0
    foreach ($dx in @(-12, -8, -4, 0, 4, 8, 12)) {
      try {
  ...
  # 형식: 히트수/63 + dy행별 '최대밝기/최대채도' (기준점 포함)
  # 분모는 실제 표본 수입니다 - dy 9행 × dx 7열 = 63 (2026-08-20 dx 촘촘화로 27→63).
  # 그물을 넓힐 때(2026-07-29) 문자열만 옛 18 로 남아 제보 로그의 히트율을 실제보다 높게
  # 보이게 했습니다 (2026-08-10 교차 리뷰). 과거 계측 주석의 '2/18'·'2/27' 등은 각 그물
  # 확대 **전**의 기록이라 그대로 둡니다.
  $script:lastPillProbe = ('기준({0},{1}) {2}/63 [{3}]' -f $refX, $refY, $hits, ($probeRows -join ' '))
  return ($hits -ge 3)
}
...
function Set-DgOptionDifficulty {
  ...
  # 사용자 조작 중이면 확정 클릭 전에 기다렸다가 **탐색·선확인부터** 다시 합니다 (2026-09-08
  # 전수 분류 - 루프 밖에서 얻은 $point 재사용 금지, 대기 중 사용자가 골랐으면 클릭 생략 - Codex)
  if (Test-UserRecentlyActive) {
    Wait-UserYieldEnd -Game $Game -Context "난이도 '$Label' 확정"
    if (-not (Resume-DgOptionDifficultyAfterYield -Game $Game -Label $Label -PointRef ([ref]$point))) { return $false }
    if (Test-DifficultySelectedAt -Game $Game -ScreenPoint $point) {
      Write-RunLog "$($script:contentTag) 난이도 '$Label' 이미 선택 확인 - 클릭 생략 (양보 후 재확인)"
      return $true
    }
  }
  Focus-Game -Game $Game
  ...
  # 재전송은 옵션 화면(제목 '구역')이 그대로일 때만, 최대 2회. 생략 원인이 사용자 조작이면
  # 회수 미소모 + 대기 + 탐색·선확인부터 (커서 실패만 2회 소모 - 기존 계약).
  ...
  # 사용자 조작 중이면 대기 후 **현재 선택 상태부터** 다시 보고, 아직 필요할 때만 최종 미전송
  # 클릭을 보냅니다 (함수 전체 재시작으로 최초·최종 예산을 초기화하지 않음 - Codex).
  $finalClickSent = $false
  while (-not $finalClickSent) {
    ...
    Focus-Game -Game $Game
    Click-ScreenPoint -X $point.X -Y $point.Y
    if ($script:lastClickPerformed) { $finalClickSent = $true }
    elseif ($script:lastClickSkipReason -ne 'user-active') { break }   # 커서 미확인 - 기존대로 확인만 이어감
  }
  Start-Sleep -Milliseconds 900
...
function Resume-DgOptionDifficultyAfterYield {
  # Set-DgOptionDifficulty 의 양보 후 재개 공통부: 옵션 화면에서 난이도 글자를 **다시 탐색**해
...
function Set-DgToggleCard {
  ...
  for ($setTry = 1; $setTry -le $setTryMax; $setTry++) {
    ...
    # 배율 추가는 답이 아니었습니다 (2026-09-16 검토): 콘솔 프레임 12장에 배율 2 는 8장만 '선택됨'을 읽고
    # 4장은 빈 판독 - 반투명 UI 뒤 배경 밝기의 미세 차(RGB합 차 최대 ~50)에 흔들리는 칼끝 판독. 근본 대응은
    # 아래 판정식의 조각 '태되' (같은 12장에서 배경과 무관하게 결정적으로 나오는 문자열).
    $cardScales = @(5)
    ...
    $matchedFragmentWord = ''   # '태되' 조각으로만 성립한 매치의 단어 (회전마다 초기화 - 진단 로그용)
    foreach ($cardScale in $cardScales) {
      foreach ($cardRegion in $cardRegions) {
        ...
        foreach ($cardWord in $cardWords) {
          ...
          # '태되' = 콘솔 렌더 깨짐 실측 (2026-09-16 - RDP→콘솔 전환 실사고의 근본 대응): 콘솔 상태(창 1024x577)에서
          #   '선택됨'이 결정적으로 깨져 배율 5·3·4 × 주·보조 6판독 전부 불가('RdEHEl'/'人에E}1되'/',k•i태되'/
          #   '人1태되' - 09-07 2장·09-14 진단 10장 보관 프레임 12장이 한 글자도 다르지 않음) → 소모량 근거 블라인드
          #   해제 폴백에만 의존(회차당 약 8초). '태되'('택됨'의 아랫획 소실)는 12장 전부 배율 3 주(3번째 판독)에서
          #   나오고, 보관 캡처 118장 × 카드 2 × 배율 5·3·4·2 × 주·보조 전 판독 오탐 스윕에서 '태되' 출현 21건이
          #   전부 진짜 '선택됨'(현행 판정 selected 9 + 콘솔 12), '도전' 9건(루팅 8·사냥터 1)에서 출현 0 = 충돌 0.
          #   콘솔 '도전' 상태 프레임은 없어(폴백 뒤 소모량으로만 확인) 아래 [진단] 로그로 다음 무인 회전에서 관측.
          if ($wordText.Contains('됨') -or $wordText.Contains('선택') -or $wordText.Contains('선태')) {
            $isSelected = $true
          } elseif ($wordText.Contains('태되')) {
            $isSelected = $true
            $matchedFragmentWord = $wordText
    ...
    if ($matchedFragmentWord) {
      # 기존 세 조각('됨'/'선택'/'선태')이 전부 불일치하고 '태되'로만 selected 가 성립한 회전 = 콘솔 렌더 대응 경로.
      # 이 로그가 입증하는 것은 '태되만으로 판정했다'는 사실까지입니다 (설계 리뷰 조건 - 콘솔 '도전' 프레임의 음성
      # 검증 대체가 아님: 해제 클릭 뒤 재확인에서 찍혀도 클릭이 게임에 반영되지 않아 진짜 선택됨이 남은 경우와
      # '도전' 오독을 이 로그만으로는 가르지 못함). 실측 단어를 그대로 남겨 다음 무인 회전의 관측 자료로 씁니다.
      Write-RunLog "[진단] $Label 버튼 글자를 '태되' 조각으로 판독: '$matchedFragmentWord' (콘솔 렌더 대응)"
    }
...
function Invoke-DgBackToSelection {
  ...
  for ($backTry = 1; $backTry -le 10; $backTry++) {
    ...
    # 사용자 조작 중이면 클릭하지 않고 기다립니다 (회전·클릭 예산 미소모 - 2026-09-08 전수 분류).
    # 루프 상단의 제목 재판독이 재개점 - 대기 중 사용자가 돌아가 있으면 클릭 없이 성공
    if (Test-UserRecentlyActive) {
      Wait-UserYieldEnd -Game $Game -Context '선택 화면 복귀'
      $backTry--
      continue
    }
    $backInputs++
...
function Invoke-NormalDungeonCycle {
  ...
  if ($onOptionsScreen -or $onSelectionScreen) {
    ...
    if ($deepMode -ne $deepTabMark) {
      ...
      for ($tabTry = 1; $tabTry -le 3 -and -not $tabSwitched; $tabTry++) {
        # 사용자 조작 중이면 클릭하지 않고 기다린 뒤 **목표 탭 도달 여부부터** 확인합니다 (사용자가
        # 탭을 눌렀을 수 있음 - Codex). 시도 미소모 (2026-09-08 전수 분류 - 양보 회전이 3회를
        # 소진해 코드 4 정지하는 같은 기전)
        if (Test-UserRecentlyActive) {
          Wait-UserYieldEnd -Game $Game -Context "'$tabTargetLabel' 탭 전환"
          $tabTry--
          if (Test-DgTabProbeMatchesMode -ProbeText ([string](Get-DgStageEnterButtonText -Game $Game)) -DeepTab $deepMode) { $tabSwitched = $true }
          continue
        }
        ...
        if (-not $script:lastClickPerformed -and $script:lastClickSkipReason -eq 'user-active') {
          # 경합 창 백업 - 같은 양보 계약 (대기 → 도달 확인 → 재탐색부터)
          Wait-UserYieldEnd -Game $Game -Context "'$tabTargetLabel' 탭 전환"
          $tabTry--
          if (Test-DgTabProbeMatchesMode -ProbeText ([string](Get-DgStageEnterButtonText -Game $Game)) -DeepTab $deepMode) { $tabSwitched = $true }
          continue
  ...
  if ($script:customMode) {
    ...
    for ($diffTry = 1; $diffTry -le 3; $diffTry++) {
      # 사용자 조작 중이면 클릭하지 않고 기다립니다 (시도 미소모 - 2026-09-08 전수 분류).
      # 다음 회전이 글자 탐색부터 다시 = 재판독 (Focus-Game 의 ALT 주입 전에 양보)
      if (Test-UserRecentlyActive) {
        Wait-UserYieldEnd -Game $Game -Context "난이도 '$ndDifficulty' 선택"
        $diffTry--
        continue
      }
      ...
      if (-not $script:lastClickPerformed -and $script:lastClickSkipReason -eq 'user-active') {
        # 경합 창 백업 - 클릭 로그·확인 호출 전에 판정 (Codex)
        Wait-UserYieldEnd -Game $Game -Context "난이도 '$ndDifficulty' 선택"
        $diffTry--
        continue
      }
  ...
  if ($difficultyPoint) {
    ...
    $ndDiffClicked = $false
    $ndDiffStopReason = '커서 확인 실패 지속'
    $ndDiffRefind = $false
    for ($ndDiffTry = 1; $ndDiffTry -le 3; $ndDiffTry++) {
      # 사용자 조작 중이면(또는 클릭 직전 경합으로 생략됐으면) 대기 후 **선택 화면 확인 → 난이도
      # 재탐색**부터 다시 합니다 (루프 밖에서 얻은 좌표 재사용 금지 - Codex. 2026-09-08 전수
      # 분류). 이 회전은 대기 후 그대로 클릭까지 가므로 시도를 소모하지 않고, 커서 미확인만
      # 기존대로 3회를 소모합니다.
      if ($ndDiffRefind -or (Test-UserRecentlyActive)) {
        ...
        $difficultyPoint = $ndDiffFresh
      }
      Focus-Game -Game $Game
      Click-ScreenPoint -X $difficultyPoint.X -Y $difficultyPoint.Y
      if ($script:lastClickPerformed) { $ndDiffClicked = $true; break }
      if ($script:lastClickSkipReason -eq 'user-active') { $ndDiffTry--; $ndDiffRefind = $true; continue }
      Start-Sleep -Milliseconds 700
      if (-not ((([string](Get-DgStageEnterButtonText -Game $Game)) -replace '\s', '').Contains('진입'))) { break }
    }
    if (-not $ndDiffClicked) {
      Write-RunLog "[완료] 난이도 '$ndDifficulty' 클릭을 전송하지 못했습니다 ($ndDiffStopReason) - 오난이도 판 방지를 위해 정지합니다"
      exit 4
    }
    Write-RunLog "$($script:contentTag) 난이도 '$ndDifficulty' 클릭"
    Start-Sleep -Milliseconds 900
  ...
  for ($selectionRound = 1; $selectionRound -le 2 -and -not $selectionReady; $selectionRound++) {
    ...
    for ($stageTry = 1; $stageTry -le 4; $stageTry++) {
      ...
      # 사용자 조작 중이면 클릭하지 않고 기다립니다 (시도 미소모 - 2026-09-08 전수 분류).
      # 대기 후에는 클릭 지점 계산부터 다시 (아래 '매 시도마다 다시 계산' 계약이 곧 재판독)
      if (Test-UserRecentlyActive) {
        Wait-UserYieldEnd -Game $Game -Context "구역 ${ndStage} 선택"
        $stageTry--
        continue
      }
      ...
      if (-not $script:lastClickPerformed -and $script:lastClickSkipReason -eq 'user-active') {
        # 경합 창 백업 - 클릭 결과 판독·후보 제외·오선택 복구로 넘어가지 않고 대기 후 좌표부터 다시 (Codex)
        Wait-UserYieldEnd -Game $Game -Context "구역 ${ndStage} 선택"
        $stageTry--
        continue
      }
  ...
  if ($ndMatching -eq '우연한 만남') {
    ...
    if ($toggleState -ne 'on') {
      # 사용자 조작 중이면 대기 후 **앵커·상태를 다시 읽고**(사용자가 켰을 수 있음) 필요할 때만
    ...
    for ($enterTry = 1; $enterTry -le 5; $enterTry++) {
      ...
      # 사용자 조작 중이면 클릭하지 않고 기다립니다 (시도 미소모 - 2026-09-08 전수 분류). 대기 후에는
      # 옵션 화면을 벗어났는지 먼저 봅니다 - 사용자가 직접 입장시켰을 수 있음 (아래 클릭 후 판정과
      # 같은 조건). 아직 옵션 화면이면 다음 회전이 캡처·IME 확인부터 다시 (Codex 재개 순서)
      if (Test-UserRecentlyActive) {
    ...
    if ($toggleState -eq 'on') {
      ...
      if ($toggleState -ne 'on') {
        ...
        Write-RunLog "$($script:contentTag) '우연한 만남' 토글 꺼짐 확인 (양보 중 전환됨)"
      } else {
        Start-Sleep -Milliseconds 900
  ...
  if ($script:dgLastRun -and -not $script:customCleanupOnly) {
    ...
    while ((Get-Date) -lt (Get-YieldAdjustedDeadline -Deadline ([ref]$fieldDeadline) -SeenYieldMs ([ref]$fieldSeenYieldMs))) {
      ...
      if ($exitStep -eq 'wait') {
        # 아무 화면도 인식 못 한 회전에만 블로커 팝업을 치웁니다 (2026-09-13 14:22 관측: 나가기 뒤 필드에 도착했는데
        # '회복 물약이 부족합니다' 구매 팝업이 화면 전체를 어둡게 덮어 HUD 판독이 막힘 → 매 회전 'wait' → 40초 뒤
        # '필드 복귀를 확인하지 못했습니다' 경고. 캡처 오프라인 재현: HUD·재시도·탐험 문구 전부 미탐 = 'wait', 스윕의
        # '닫기' 탐색은 (508,616) 적중). 다른 복귀 대기 루프들은 이 스윕을 배선하고 있었고 이 루프만 빠져 있었습니다.
        # 'wait' 에만 거는 이유: 결과 화면이 인식된 회전(reclick)은 재클릭이 답이고, 스윕의 '닫기' 탐색 영역($rgPopupClose)이
        # 결과 화면 버튼 줄과 겹치므로 관측 없는 경로를 새로 열지 않습니다(규칙 8 - 결과 화면 캡처 26장에서 '닫기'
        # 오탐 0건이라 위험이 실측된 것은 아님). 필드·탐험 팝업 회전에는 불필요. 닫은 회전은 같은 프레임으로 판정하지
        # 않고 continue 로 재판독(형제 '다시 하기' 복귀 대기와 같은 계약). 구매 팝업 직접 닫기의 user-active 취소는 스윕이
        # 기다린 뒤 $true 를 돌려주고 그 양보는 while 조건의 차분 연장이 흡수합니다 (스윕이 위임하는 보상·협동·보드·네트워크
        # 4함수는 기다리지 않음 - 이 루프에서 관측된 적 없는 화면이라 별건, 이슈 목록). 무팝업 wait 회전당 OCR 약 7회가
        # 늘지만(재시작·닫기·하단·협동·보드 2·네트워크) 실측 전환(14:04, 재클릭→필드 확정 9초)에서 wait 회전은 1~2회라
        # 40초 예산 안입니다 (설계 합의).
...
function Invoke-HuntingGroundCycle {
  ...
  if ($difficultyPoint) {
    ...
    $htDiffClicked = $false
    $htDiffStopReason = '커서 확인 실패 지속'
    $htDiffRefind = $false
    for ($diffDispatchTry = 1; $diffDispatchTry -le 3; $diffDispatchTry++) {
      # 사용자 조작 중이면(또는 클릭 직전 경합으로 생략됐으면) 대기 후 **첫 화면 확인 → 난이도
      # 재탐색**부터 (루프 밖 좌표 재사용 금지 - Codex. 2026-09-08 전수 분류). 대기 후 그대로
      # 클릭까지 가므로 시도 미소모, 커서 미확인만 기존대로 3회 소모 - 던전 비커스텀과 동일 계약
      if ($htDiffRefind -or (Test-UserRecentlyActive)) {
        ...
        $htDiffFresh = Find-GameTextPoint -Game $Game -ReferenceX $rgHtDifficulty[0] -ReferenceY $rgHtDifficulty[1] `
          -RegionWidth $rgHtDifficulty[2] -RegionHeight $rgHtDifficulty[3] -Scale 4 -SearchText $difficultySearch -ExactText $difficultyKey
        if (-not $htDiffFresh) { $htDiffStopReason = '양보 후 난이도 글자 재탐색 실패'; break }
        $difficultyPoint = $htDiffFresh
      }
      Focus-Game -Game $Game
      Click-ScreenPoint -X $difficultyPoint.X -Y $difficultyPoint.Y
      if ($script:lastClickPerformed) { $htDiffClicked = $true; break }
      if ($script:lastClickSkipReason -eq 'user-active') { $diffDispatchTry--; $htDiffRefind = $true; continue }
      Start-Sleep -Milliseconds 700
      if (-not (Find-HtEntryButtonPoint -Game $Game)) { break }
    }
    if (-not $htDiffClicked) {
      Write-RunLog "[완료] 난이도 '$htDifficulty' 클릭을 전송하지 못했습니다 ($htDiffStopReason) - 오난이도 판 방지를 위해 정지합니다"
      exit 4
    }
  ...
  if ($htMatching -eq '파티찾기') {
    ...
    for ($enterTry = 1; $enterTry -le 4; $enterTry++) {
      ...
      # 사용자 조작 중이면 클릭하지 않고 기다립니다 (시도 미소모 - 던전 입장과 같은 계약). 대기 후
      # 입장됐는지는 '입장 버튼 소멸 + 팝업 아님'(아래 클릭 후 판정과 동일 조건)으로만 인정
      if (Test-UserRecentlyActive) {
        Wait-UserYieldEnd -Game $Game -Context '입장하기 클릭'
        $enterTry--
...
$lifeTargetVariants = @{
  ...
  '화살꽃'     = @('호b살꽃', '화살것')
  # '斟락나무': 2026-08-14 네이티브 1908 실측 (나무 베기 7/8 정지 - '벼'가 한자 '斟'으로
  # 깨져 스크롤 탐색이 목록 끝까지 못 찾음. 같은 진단의 나머지 5행은 전부 정상 판독)
  '벼락나무'   = @('벍락나무', '벼락나부', '벍락나부', '斟락나무')
  '운철광맥'   = @('문철광맥', '운철괌맥', '문철괌맥')
  # '광'→'고十' 분해 계열 (2026-08-14 네이티브 1908 실측 - 은 광맥 미발견 정지의 진단 캡처
  # 한 장에 6건 동시 관측. 제목 전용 표의 '고十OH' 와 같은 깨짐이 행 이름에도 출현.
  # 석탄·백동·운철 앵커와 목록 순서로 대상 대응 확정 - Codex 합의로 일괄 등록)
  '은광맥'     = @('으광맥', '은괌맥', '으괌맥', '으고十맥')
  '동광맥'     = @('도고十臼H')
  '백금광맥'   = @('BH금광맥')
  '광맥'       = @('고十맥')
  '철광맥'     = @('*40고十맥')
  ...
  '고요한빛무리' = @('고요한무리')
  # 양털 깎기 행 깨짐 (2026-08-15 복슬 양 미발견 정지의 진단 캡처 + 오프라인 s4/s5 재현.
  # 복슬 양은 이름이 통째 소실(레벨 열만 읽힘)이라 이형 등록 자체가 불가 - **격자 추론이
  # 유일한 경로**고, 아래 등록은 그 앵커(3개 = order-strong)를 살리기 위한 것.
  # '고스0'은 제목 전용 표 등록을 유지하고 여기엔 행 실측 '-고스0'만 (Codex 조건 -
  # 제목 전용 이형이 공용 매칭에 새지 않음 가드 보존)
  '곱슬양'     = @('-고스0')
  '구름털양'   = @('丁-터00')
  '먹구름양'   = @('머그르OE', '그르OE')
}
...
$lifeDetailDescSignatures = @{
  '물'   = @('마실수있는맑은물')
  # 우물: 2026-08-14 18:26 실사고 - 근거 index(이름·격자 실패)로 찾은 우물 팝업이 제목 통째
  # 소실 + 스킬명 '일상채집'이 '일상치1집'으로 깨져 index 분기 확인까지 실패, 3회전 소진.
  # 판독 원문(3회 동일): '채집물일상치1집레벨1이상깨끗한지하수를모아둔곳.빈병으로물을뜰수
  # 있다.가까운위치찾기'. 과거 1272 전수 배치 실측에도 같은 설명문이 그대로 읽혀(진리표
  # '丁亞치|…' 케이스) 두 기하에서 안정. 물의 설명과 겹치지 않음(오탐 검증).
  '우물' = @('깨끗한지하수를모아둔곳')
}
...
# 골드 영역 왼쪽 확장 935 → 910 (2026-09-15, 이슈 목록 2절 '시작 골드 앞자리 소실' — 09-13 20:25 / 09-14 19:25 관측):
#   골드 숫자는 오른쪽 정렬이라 글자 폭(비례 폰트)에 따라 흰 글자 왼쪽 끝이 x 933~949 사이에서 움직이는데(감시 프레임
#   5,015장 실측, 최빈 936), 옛 영역은 왼쪽 여유가 0~1px 라 끝이 934~936 이면 첫 글자가 잘려 '?9,782,324' → 9,782,324.
#   같은 값이 566초간 2,707프레임 연속 동일하게 잘리는 결정적 현상이라 '2연속 동일 값 확정'으로는 못 거릅니다.
#   전수 비교(워커 판독 경로 그대로): 935 오독 3,115/5,015 · 930 은 아이콘 반쪽이 '2'로 융합(25) · 910 오독 3(전부 아래
#   '기' 유형) · 905 35 · 900 51. 아이콘(x≈915~933)을 통째로 넣고 글자 최소 경계에서 23px 여유 — 아이콘은 '@'/')' 접두로
#   읽혀 파서가 떼어냅니다. 1908 창은 비율 환산(크롭 1402→1365px)에 다른 HUD 요소 없음(설계 리뷰 확인). 오른쪽 경계 1085 불변.
#   진리표 tests\test_nyan_gold_offline.ps1, 자료 던전이미지\실측기록\20260915_냥코인_골드_앞자리잘림_프레임\.
$rgNyanGold   = @(910, 40, 175, 45)    # 우상단 골드 잔량 (지출 = 시작-현재 차감)
...
function Get-NyanNumberValue {
  ...
  param([string]$Text)
  $raw = [string]$Text
  # 첫 숫자와 마지막 숫자 **사이**에 숫자·구분자(쉼표/마침표/공백) 외 글자가 있으면 판독 불능(-1)로 돌립니다 (2026-09-15):
  # 감시 프레임 5,015장 전수에서 중간 글자 '2,'가 '기'로 읽히는 오독('02965기124'/'@29.59기524'/'03006기324')이 3~35장
  # 나왔는데, 옛 규칙(비숫자 전부 제거)은 이걸 한 자리 빠진 **틀린 값**으로 돌려 골드 상한 계산을 어긋나게 했습니다.
  # 후보 3영역의 판독 15,045건 중 정상 분류 14,956건에 내부 잡음이 0건이고 냥코인 잔량 원문(')14,021,217')도 접두 잡음뿐이라 무해 — 접두·접미 잡음
  # (아이콘 '@'/')'/'0')은 그대로 떼어냅니다. 틀린 값 대신 -1 이면 호출부의 재시도(시작 골드 5회)·실패 카운터 경로를 탑니다.
  $firstDigit = [regex]::Match($raw, '[0-9]')
  if (-not $firstDigit.Success) { return [int64](-1) }
  $lastDigit = [regex]::Match($raw, '[0-9](?=[^0-9]*$)')
  $core = $raw.Substring($firstDigit.Index, $lastDigit.Index - $firstDigit.Index + 1)
  if ($core -match '[^0-9.,\s]') { return [int64](-1) }
...
function Test-NyanSameTag {
  ...
  foreach ($tag in @($Tags)) {
    # null 항목은 [int] 캐스팅이 X=0 Y=0으로 읽혀 (0,0) 앵커와 일치 판정됩니다 - 2026-08-15
    # 실사고: 빈 판독($null)이 @()에서 1칸 배열이 되며 "0,0에 가격표 잔존"으로 8초 교착 후
    # 조건부 정지 (오프라인 재현 확정. 호출부 @() 수정과 별개로 순수 함수도 계약을 지킴)
...
function Test-NyanCoinSuspect {
  ...
  # 도달 오판. 실측 아이콘 오독 3종: '0' 무해 / '@' 제거됨 / '9' 자릿수 증가로 치명).
  # 냥코인은 현상금(실측 수천~수만)으로만 늘고 이 흐름에 소비 경로가 없으므로,
  # 직전 확정값 대비 '2배+10만 초과 급증'과 '감소'는 모두 오독으로 봅니다 (Codex 합의 -
  # 감소를 채택하면 앞자리 소실 오독이 확정된 뒤 정상값이 급증 가드에 영구 차단됩니다).
  param([int64]$Previous, [int64]$Current)
  if ($Previous -lt 0 -or $Current -lt 0) { return $false }   # 기준 없음/판독 실패는 별도 경로
  if ($Current -gt ($Previous * 2 + 100000)) { return $true }
  if ($Current -lt $Previous) { return $true }
  return $false
}

function Get-NyanCoinCorrection {
  # 냥코인 접두 오독 자기 보정 (순수 - 진리표 대상. 2026-08-15 07:11 실사고: 아이콘 '9' 접두
  # 오독이 '지속형'이라 - 한 번 깨지면 같은 화면 상태 동안 계속 반복 - 급변 가드가 8연속
  # 의심(6.4초) → 조건부 정지. 실측 98,842,217 = '9' + 8,842,217(직전 확정값 그대로)).
  # 실측된 형태만 보정합니다 (Codex 조건 2026-08-15):
  #  - 첫 자리가 '9'일 때만 (다른 접두 숫자는 미관측 - 규칙 8)
  #  - 접미부 선행 0 거부 ('908,842,217'처럼 2자리 접두를 조용히 제거하는 형태 방지)
  #  - 뗀 값이 [직전 확정값, 직전+허용 증가폭] 안일 때만 채택. 기본 10만 = 현상금 실측
  #    수천~수만의 보수 상한 (수 초 간격 판독에서 10만 초과 증가는 게임상 불가능).
  #    체인 구매(잔량 판독 간 최대 3구매) 뒤에는 정당한 누적 증가가 10만을 넘을 수 있어
  #    호출부가 '10만 × 판독 간 구매 수'로 배치 인식시킵니다 (2026-08-15 Codex 반례 반영 -
  #    안 하면 체인 직후 '9' 오독 시 보정 실패 → 8연속 fail-closed).
  # 잔여 위험(문서화): 접두+본체 복합 오독이 우연히 범위 안이면 오채택 가능하나, 그 경우
  # 다음 정상 판독이 감소로 걸려 fail-closed 정지로 귀결 (안전 측 실패 - Codex 검토).
  param([int64]$Previous, [int64]$Current, [int64]$AllowedGain = 100000)
  if ($Previous -lt 0 -or $Current -le 0) { return [int64](-1) }
  $digits = $Current.ToString([Globalization.CultureInfo]::InvariantCulture)
  if ($digits.Length -lt 2 -or $digits[0] -ne [char]'9') { return [int64](-1) }
  $suffix = $digits.Substring(1)
  if ($suffix.Length -gt 1 -and $suffix[0] -eq [char]'0') { return [int64](-1) }
  $candidate = [int64]$suffix
  if ($candidate -lt $Previous) { return [int64](-1) }
  if (($candidate - $Previous) -gt $AllowedGain) { return [int64](-1) }
  return $candidate
}
...
function Read-NyanPriceTags {
  ...
  # 주의: 함수 반환은 파이프라인에서 풀립니다(규칙 3) - 1개면 Hashtable 맨몸, 0개면 null이
  # 호출부에 도착하므로 **호출부에서 반드시 @()로 수집**해야 합니다 (Get-GameRegionOcrWords 와
  # 같은 규약). 2026-08-15 실사고: 맨몸 할당 상태로 [0] 인덱싱해 firstTag가 null → 클릭
...
function Invoke-NyanMerchantRun {
  ...
  $boardPurchases = 0            # 이 판에서 확인된 구매 수 (판당 뽑기 한도 5 - 아래 실측 주석)
  $pendingBoardTags = $null      # REROLL_WAIT가 남긴 확정 가격표 판독 (일회성 재사용)
  $pendingBoardTagsClock = $null
  $coinFailStreak = 0
  $goldFailStreak = 0
  $purchaseCount = 0
  while ($true) {
    ...
    if ($coinSuspect) {
      ...
      # (2026-08-15 07:11 실사고). 보정 성공 시 그 값을 정상 판독으로 취급해 아래
      # 기존 경로(확정 갱신·목표 2연속 검사)에 그대로 합류시킵니다 (Codex 배선 합의).
      $correctedCoin = Get-NyanCoinCorrection -Previous $lastCoinValue -Current $coinNow `
        -AllowedGain (100000 * [int64][Math]::Max(1, $coinReadGapPurchases))
      if ($correctedCoin -ge 0) {
        Write-RunLog ("[진단] 냥코인 접두 오독 보정 - 판독 {0:N0} → {1:N0} 채택 (직전 {2:N0})" -f $coinNow, $correctedCoin, $lastCoinValue)
        $coinNow = $correctedCoin
        $coinSuspect = $false
      }
    }
    if ($coinNow -ge 0 -and -not $coinSuspect) {
      $coinFailStreak = 0
      $coinReadGapPurchases = 0   # 잔량 확정 - 보정 상한 배치를 기본(10만)으로 복귀
      ...
      # 실패/급변 의심 판독은 확정값을 갱신하지 않고 **이번 주기 구매도 진행하지 않습니다**
      # (Codex 조건: 유효한 잔량 확인 없이는 클릭 금지). 지속되면 fail-closed 정지.
      if ($coinSuspect) {
        Write-RunLog ("[진단] 냥코인 판독 급변 의심 - 무시합니다 (직전 {0:N0} → 판독 {1:N0})" -f $lastCoinValue, $coinNow)
      }
      $coinFailStreak++
      if ($coinFailStreak -ge 8) {
        Write-RunLog '[완료] 냥코인 잔량 판독이 계속 실패하거나 급변합니다 - 화면을 확인해 주세요 (조건부 정지)'
        exit 4
      }
      Start-Sleep -Milliseconds 800
      continue
    ...
    if ($nyanGoldLimitEnabled) {
      ...
      if ($goldNow -ge 0) {
        ...
        # 상한이 켜져 있는데 잔량을 못 읽었으면 이번 회전은 구매하지 않습니다 (2026-09-15 설계 리뷰: 예전엔 실패 1~7회째에
        # 그대로 구매 경로로 내려가 상한 검사 없이 사고, 체인 판정도 옛 $lastGoldValue 를 썼음). 냥코인 실패 분기와 같은 계약 -
        # 유효한 잔량 확인 없이는 클릭 금지, 지속되면 위 8회 fail-closed.
        Start-Sleep -Milliseconds 800
        continue
      }
    }
    # 가격표 탐색 (카드 존 한정. @() 필수 - 반환이 풀려서 도착: 규칙 3, 함수 주석 참고).
    # REROLL_WAIT가 남긴 확정 판독(pendingBoardTags)이 신선하면 재사용해 OCR 1회 생략 -
    # 새 판 첫 클릭 가속 (Codex 조건: 일회성 + 나이 3초 상한 + 캡처 실패 시 폐기. 재화 판독
    # 실패로 주기를 건너뛰면 나이 초과로 자연 폐기됨)
    if ($null -ne $pendingBoardTags -and -not $script:screenCaptureFailing -and
      $pendingBoardTagsClock.Elapsed.TotalSeconds -le 3) {
      $tags = $pendingBoardTags
    } else {
    ...
    if (@($tags).Count -gt 0 -and $boardPurchases -lt 5) {
      ...
      # + 체인 구매 (2026-08-15 사용자 '1~2초' 요청 - Codex 3조건 반영 승인): 경계 원거리일 때
      # 소멸 확인 판독($tagsNow = 직전 1초 내 실측 좌표)으로 다음 카드를 루프 상단 재검(잔량
      # OCR ~1초) 없이 최대 2장 연속 구매. 완화되는 것은 '2연속 안정'을 판 내 카드 정지 실측
      # (75구매+ - READY 자체가 이에 의존해 무결점)에 근거해 체인 중 최신 단일 판독으로 갈음
      # 하는 것뿐이고, 클릭마다 소멸 확인은 그대로(검증 후 조작 유지). 유령 태그(OCR 노이즈)
      # 클릭은 빈 자리 클릭이라 무해 - 단 그 무해 논증은 $purchaseCount 가 로그 표기 외 어떤
      # 종료/상한 판단에도 쓰이지 않아야 성립 (Codex 무해 조건 - 배선 가드가 지킴).
      ...
      while ($true) {
        ...
        # 피드백: 좌표가 '474,584골드'처럼 읽혀 혼동 + 클릭마다 한 줄은 화면 소음. 화면에는
        ...
        $boardPurchases++
        # 체인 계속 판정: 경계 원거리 + 남은 카드 + 예산 + 판 한도(5회) 미달. 경계 근접·
        # 판 소진·예산 소진·한도 도달이면 루프 상단으로 복귀해 기존 재검을 치릅니다.
        $farFromTarget = ($lastCoinValue -ge 0 -and ($nyanTargetCoins - $lastCoinValue) -gt 600000)
        $farFromGoldLimit = (-not $nyanGoldLimitEnabled -or ($lastGoldValue -ge 0 -and ($nyanGoldLimit - ($startGold - $lastGoldValue)) -gt 300000))
        if ($chainBudget -gt 0 -and $farFromTarget -and $farFromGoldLimit -and @($tagsNow).Count -gt 0 -and $boardPurchases -lt 5) {
          $chainBudget--
          $firstTag = $tagsNow[0]
          Test-NyanUntilReached   # 값싼 검사라 체인 클릭마다 유지 (Codex 조건 - PURCHASE_WAIT 최악 8초×2 누적 유예 방지)
          Start-Sleep -Milliseconds 200   # 공개 연출 완충 - 클릭이 연출에 먹혀도 소멸 확인 + 4초 재클릭이 상태 기반 회수
          continue
        }
        break
      }
      # 공개 연출 + 재화 표기 갱신 안정 대기 - 경계 부근에서만 1200ms 전체, 멀 때는 300ms.
      # (2026-08-15 사용자 추가 단축 요청 → Codex 승인 '명시적 계약 완화') stale-low 판독의
      # 피해는 경계를 넘는 순간을 놓치는 것뿐이라, 경계에서 멀면 낮게 읽혀도 '계속 진행'
      # 판정으로 무해합니다. 판독 자체는 매 주기 하므로 남는 위험은 표기 지연이 여러 주기
      # (≈7초 = 여유폭 30만 ÷ 현상금 보수 상한 10만 × 주기 ~2.3초)를 넘는 미관측 상황뿐.
      # 그 경우 과구매가 시작되며 표기 갱신·다른 종료 조건까지 이어질 수 있음 - 관측 범위
      # 기준 예상 피해는 1~2장(수천~수만 골드)이고 이는 하드 상한이 아님 (Codex 문구 조건).
      # 실측 75구매+에서 표기 지연이 1주기(~3초)를 넘는 관측은 없음.
      # 여유폭: 냥코인 30만(현상금 실측 수천~수만) / 골드 10만(가격표 실측 1,600~7,800).
      # 미확정(-1)이면 보수적으로 1200ms.
      $nearTarget = ($lastCoinValue -lt 0 -or ($nyanTargetCoins - $lastCoinValue) -le 300000)
      $nearGoldLimit = $false
      if ($nyanGoldLimitEnabled) {
    ...
    # **판당 뽑기는 5회 한정** (2026-08-15 실측: 구매중 캡처의 상인 말풍선이 1장 구매 직후
    # '콩냥~ 콩냥~ 4번 남았다냥!' + 사용자 확인 "5번인데". 판은 7카드지만 5회만 구매 가능 -
    # 세 판 연속 6클릭 로그 = 5회 소진 후에도 잔존 가격표(1~2초)를 눌러 게임이 거부한 무효
    ...
    if (-not $boardLimitReached) {
      ...
      # 오판 방지), 재확인 2회는 잔량 OCR이 없는 가격표 전용 빠른 재판독으로 (2026-08-15
      # 사용자 속도 요청 - 종전에는 재확인마다 루프 상단 전체 주기를 치러 소진→다시 뽑기까지
      # 5~7초). Codex 조건 2가지: ①최소 벽시계 2초 - 세 판독이 전부 한 연출 안에 압축되는
      # 오판 방지 ②캡처 실패는 매 판독 직후 확인해 상단 복구 경로로.
      $emptyConfirmed = $false
      $emptyRechecks = 0
      $emptyClock = [System.Diagnostics.Stopwatch]::StartNew()
      while ($true) {
        if ($script:screenCaptureFailing) { break }
        Start-Sleep -Milliseconds 350
        $recheckTags = @(Read-NyanPriceTags -Game $Game)
        if ($script:screenCaptureFailing) { break }
        if (@($recheckTags).Count -gt 0) { break }
        $emptyRechecks++
        if ($emptyRechecks -ge 2 -and $emptyClock.Elapsed.TotalSeconds -ge 2) { $emptyConfirmed = $true; break }
      }
    ...
    # (전 흐름과 달리 단일 무한 루프라 회차 종료 기반 GUI측 정지도 오지 않음). Codex 조건:
    # 확인·소비를 클릭 직전 마지막 동작으로(사이에 sleep/OCR 금지), 삭제 실패여도 정지.
    if (Test-Path -LiteralPath $safeStopFlagPath) {
      Remove-Item -LiteralPath $safeStopFlagPath -Force -ErrorAction SilentlyContinue
