...
# (주간 리셋이 이 창을 자동으로 띄움). '미선' = 1810x1020 창 s3 실측 깨짐 (타 PC 1810 창).
$boardCases = @(
  @{ T = '해* 협동 미션 0'; E = $true }    # 1272 창 s3/s4 실측 (개발 PC 06:02 캡처)
  @{ T = ',는 협동 미선 0'; E = $true }    # 1810 창 s3 실측 깨짐 (1810 창 06:02 캡처)
...
# 본체: '네트워크' AND '불안정' 엄격 ('연결' 완화는 다른 연결 오류 팝업 오포섭 위험 - 리뷰).
# 실측: 2026-08-01 타 PC(1810) 캡처 3장 s3 '불안정합니다' / s4 '불안정합LI다' - 조각 생존.
$netTitleCases = @(
